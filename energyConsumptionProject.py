...
file_data_2016 = open("data_2016.csv", "r")
file_data_2017 = open("data_2017.csv", "r")
file_data_2018 = open("data_2018.csv", "r")
file_data_2019 = open("data_2019.csv", "r")

data_2016 = file_data_2016.readlines()[1:] #get rid of the headers line
...
# print(total_data[:370])

# reduced_total_data = [line.strip("\n").split(",")[1:2] + line.strip("\n").split(",")[3:] for line in total_data] #getting rid of 'pacific' and 'cal'
reduced_total_data = [line.strip("\n").replace("/", ",").replace("\"", "").split(",")[1:4] + line.strip("\n").replace("/", ",").split(",")[5:-2] for line in total_data] #getting rid of 'everything but the data, demand, and demand forecast

cleaned_total_data = [[int(val) for val in line] for line in reduced_total_data]

file_data_2016.close()
file_data_2017.close()
file_data_2018.close()
file_data_2019.close()

print(reduced_total_data[1])
print(cleaned_total_data[1])

# [month, date, day, demand, demand forecast]
for i in range(0,len(cleaned_total_data)):
	cleaned_total_data[i][2] = datetime.date(cleaned_total_data[i][2], cleaned_total_data[i][0], cleaned_total_data[i][1]).weekday() #replacing the (now useless) year with the number corresponding to the day
...
# torch.save(model, "model_configuration.pt")
#to initialize a model w/ configuration

# model = torch.load("model_configuration.pt")

file_data_2020 = open("data_2020.csv", "r")
data_2020 = file_data_2020.readlines()[1:]


reduced_data_2020 = [line.strip("\n").replace("/", ",").replace("\"", "").split(",")[1:4] + line.strip("\n").replace("/", ",").split(",")[5:-2] for line in data_2020] #getting rid of 'everything but the data, demand, and demand forecast

cleaned_data_2020 = [[int(val) for val in line] for line in reduced_data_2020]

for i in range(0,len(cleaned_data_2020)):
	cleaned_data_2020[i][2] = datetime.date(cleaned_data_2020[i][2], cleaned_data_2020[i][0], cleaned_data_2020[i][1]).weekday() #replacing the (now useless) year with the number corresponding to the day
